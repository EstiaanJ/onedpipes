use crate::{
    ...
    solvers::{
        ...
        rusanov_face_fluxes, unweighted_state,
    },
    state::State,
};
    ...
    F: FnOnce(&[State]) -> Vec<State>,
{
    ...
    let mut fallback_face_indices = Vec::new();
    ...
    for i in 0..cells.len() {
        ...
        if !is_physical(cell, gas, density_floor, pressure_floor)
            || !is_physical(right, gas, density_floor, pressure_floor)
        {
            fallback_faces += 1;
            fallback_face_indices.push(i + 1);
            predictor_is_physical.push(false);
    ...
    SolverOutput {
        ...
        face_fluxes: rusanov_face_fluxes(extended, face_areas, gas),
    }
}
